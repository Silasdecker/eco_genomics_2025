...
barplot
abline(h=mean(colSums(counts)), col="red", lwd=2)
...
apply(counts,2,mean) # 2 in the apply function does the action across columns
apply(counts,1,mean) # 1 in the apply function does the action across rows
hist(apply(counts,1,mean),xlim=c(0,10000), ylim=c(0,2000),breaks=10000)
```
### Lets see if we cant visualize counts some more

```{r}
boxplot(log2(counts + 1), las=2, outline=FALSE)
