...
### A new markdown script was created to analyze the GO-enrichment for all generations named DESeq2_to_TopGO_HW2.Rmd and it lives in: 
...
### Each generation is mapped via a bubble plot that reveals the abundance by size of circle (ggplot figure)
...
### the topGOsig_for_REVIGO.txt files were opened in an R studio editor to paste them into REVIGO 

### In excel the GO ids of the 5 lowest p-values were tabulated for less complex comparison. The idea of this is to show the most significant overexpressed biological functions for each generation. 

### REVIGO scatter plots of each generation were made and annotated with the top 5 most significant GO Terms. 
