...
library(qqman)
```
Re-make the PCA on just the red spruce samples after removing the BS
```{r}
...
```
How does PC1 scores compare with ancestory scores when BS was involved?

Test for correlation between admixture scores from the RSBS analysis and PC1 scores for the RS only analysis. Is PC1 still an axis of introgression? IS tere any evidence for introgression also along PC2?
...
ggarrange(p1,p2)
```
Read in selection statistics and calculate p-values
...
dim(p)
```
Info on the SNPs (which chromosome they are on, what position..)
...
dim(pos_filtered)
```
Make Manhattan Plot for PC1 and PC2

```{r}
plot(-log10(pval_PC1$p_PC1), col="black", cex=0.8, xlab="Locus position", main="Selection outliers: PCAngsd PC1")

plot(-log10(pval_PC2$p_PC2), col="black", cex=0.8, xlab="Locus position", main="Selection outliers: PCAngsd PC2")
```
